"""
recommendations.py — Coach Mode recommendation engine.
Looks at skill tree levels, recent seeds, recent games, and recent debates,
then returns the top 2–3 next actions for the user.
"""
...
_REASON_PHRASES: dict[str, list[str]] = {
    ...
    "repeat_weakness": [
        "This came up in two recent sessions. Worth targeting di        "You've hit this weakness before. A focused drill will help break the pattern.",
    ],
    "diversify": [
...
def _tree_progress_map(conn: sqlite3.Connection, user_id: int) -> dict[str, int]:
    rows = conn.execut       "SELECT tree_id, xp FROM skill_progress WHERE user_id = ?", (user_id,)
    ).fetchall()
    return {row["tree_id"]: int(row["xp"]) for row in rows}


def _recent_weak_trees(conn: sqlite3.Connection, user_id: int, limit: int = 3) -> list[str]:
    """Pull skill_tree_id from the most recent new/active practice seeds."""
    rows = conn.execute(
        """
...
def _recent_games_played(conn: sqlite3.Connection, user_id: int, limit: int = 10) -> list[str]:
    rows = conn.execute(
        """
        SELECT mini_game_id FROM mini_game_sessions
        WHERE user_id = ?
...
def _new_seeds_for_tree(conn: sqlite3.Connection, user_id: int, tree_id: str) -> list[dict]:
    rows = conn.execute(
        """
...
def recommend_next_actions(
    ...
    # All trees with their current level (default 0 XP = level 1 for unseen trees)
    all_trees = list(SKILL_TREES.keys())
    tree_levels = {t: tree_level_from_xp(tree_xp.get(t, 0)) for t in all_trees}

    recommendations: list[dict[str, Any]] = []
    seen_trees: set[str] = set()

    def add_rec(mini_game_id: str, tree_id: str, reason: str, seed_id: int | None = None) -> bool:
        ...
        recommendations.append({
            ...
            "treeName": SKILL_TREES.get(tree_id, {}).get("name", tr_id),
            "reason": reason,
            "seedId": seed_id,
    ...
    for tree_id in weak_trees:
        ...
        add_rec(game_id, tree_id, _pick_phrase("new_seed", seed["id"] % 3), seed["id"])

    # Priority 2: lowest-level tree that hasn't been recommended yet
    sorted_by_level = sorted(all_trees, key=lambda t: (tree_levels[t], tree_xp.get(t, 0)))
    for tree_id in sorted_by_level:
        if tree_id in seen_trees:
            continue
        game_id = _game_for_tree(tree_id)
        # Diversify: if same game played 3+ times recently, pick next game
        if recent_games.count(game_id) >= 3:
            game_id = _alternate_game_for_tree(tree_id, recent_games)
        add_rec(game_id, tree_id, _pick_phrase("lowest_tree", tree_levels[tree_id]))
...
def _alternate_game_for_tree(tree_id: str, recent: list[str]) -> str:
    """When primary game has been overplayed, suggest the secondary."""
    alternates = {
        "clash":      "speech_polish",
