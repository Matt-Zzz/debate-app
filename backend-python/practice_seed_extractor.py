...
Extracts 1-3 personalized practice seeds from a debate transcript + rubric.
...
def _pick_weakest_sentence(sentences: list[str], scorer) -> tuple[str, float] | None:
...
_seeds:
    ...
    # ── Rule 1: Expression — hedging / vagueness (lowest-hanging fruit) ──────
    if "expression" not in used_trees:
        ...
        best = hedge_hit if (hedge_hit and (not vague_hit or hedge_hit[1] >= vague_hit[1])) else vague_hit
        if best:
            excerpt, score = best
            label = "hedged_claim" if _hedge_score(excerpt) > _vague_score(excerpt) else "vague_claim"
            note = (
        ...
        prompt = (
            ...
            add_seed(make_seed(
                "speech_polish", "expression", excerpt, label, note,
                min(0.92, 0.55 + score), prompt, "easy",
            ))
    ...
    if "weighing" not in used_trees:
        ...
        if weighing_score < weighing_max * 0.5:
            # Find a sentence that names an impact but doesn't weigh it
            impact_sentences = [
                s for s in all_sentences
                if _IMPACT_WORDS.search(s) and not _WEIGHING_WORDS.sears:
                # Prefer shorter sentences (likely single claims)
                excerpt = min(impact_sentences, key=len)
                add_seed(make_seed(
                    "impact_ranking", "weighing", excerpt,
                    "missing_weighing",
                    "You named a harm but didn't explain why it matters more than the other side's benefit. "
                    "That's the difference between saying 'this is bad' and winning on impact.",
                    0.78,
                    "Explain why this harm outweighs the opposing benefit. "
                    "Use magnitude, probability, or irreversibility.",
                    "medium",
                ))
    ...
    if "rebuttal" not in used_trees and len(user_turns) >= 2:
        ...
        clash_max = breakdown.get("clash", {}).get("max",d a turn that should be a rebuttal but doesn't mention the opponent
            rebuttal_turns = [
                t for t in user_turns[1:]
                if not _OPPONENT_WORDS.search(t.get("text", ""))
            ]
            if rebuttal_turns:
                excerpt = _split_sentences(rebuttal_turns[0].get("text", ""))
                if excerpt:
                    add_seed(make_seed(
                        "fallacy_hunt", "logic",
                        excerpt[0],
                        "no_opponent_clash",
                        "In this rebuttal turn you didn't engage with what your opponent actually said. "
                        "Practicing fallacy detection trains you to spot weak moves — in theirs and yours.",
                        0.71,
                        "Identify what logical flaw lets you dismiss this line of reasoning quickly.",
                        "medium",
                    ))

    # ── Rule 4: Logic — absolute claim with no support ────────ogic" not in used_trees:
        absolute_sentences = [
            s for s in all_sentences
        ...
        if absolute_sentences:
            ...
            add_seed(make_seed(
                "fallacy_hunt", "logic", excerpt,
                "unsupported_absolute",
                "You used an absolute claim without backing it up. "
                "Strong opponents will immediately demand a counterexample, and you have no defence.",
                0.74,
                "What fallacy does this absolute claim risk committing? "
                "Then find the version of this argument that doesn't need to be absolute.",
                "medium",
            ))

    # ── Rule 5: Clash — low structure score, no clear thesis ─────────────────
    if "clash" not in used_trees:
        ...
        if structure_score < structure_max * 0.4 and all_sentences:
            # Use the very first sentence the user said as the excerpt
            excerpt = all_sentences[0]
            add_seed(make_seed(
                "clash_point_picker", "clash", excerpt,
                "unclear_thesis",
                "Your opening didn't make the central clash clear. "
                "Before you can win an argument you need to pin down exactly what you're disagreeing about.",
                0.68,
                "What is the single most important disagreement in this debate? "
                "Pick it — then build every argument around that.",
                "easy",
            ))
...
def seeds_to_db_rows(
    ...
    now = datetime.now(timezone.c).isoformat()
    rows = []
    for seed in seeds:
        rows.append({
            "user_id": user_id,
            ...
            "status": "new",
            "created_at": now,
            # non-DB fields kept for API response
