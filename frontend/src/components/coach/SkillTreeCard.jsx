...
const S = {
  ...
  name: (active) => ({ fontSize: "14px", fontWeight: 600, color: active ? "#fff" : "#1a1a1a" }),
  level: (active) => ({
  ...
  desc: (active) => ({
    fon   height: "4px", borderRadius: "2px",
  ...
  xpRow: { display: "flex", justifyContent: "space-between", marginTop: "4px" },
  xpText: (active) => ({ fontSize: "10px", fontFamily: "'DM Mono', monospace", color: active ? "rgba(255,255,255,0.4)" : "#ccc" }),
};
  ...
  return (
    <div style={S.card(active)} onClick={onClick}>
      <div style={S.top}>
        ...
        <span style={S.name(active)}>{tree.name}</span>
        <span style={S.level(active)}>Lv {tree.level}</span>
      </div>
      ...
      {!tree.maxLevel && (
        <div style={S.xpRow}>
          <span style={S.xpText(active)}>{earned} / {total} XP</span>
          <span style={S.xpText(active)}>{tree.xpToNext} to next</span>
        </div>
      )}
    </div>
  );
