    ...
    <div style={pageWrap}>
      <div style={{ ...heroCard, marginBottom: "18px" }}>
        ...
        >
          <div>
            ...
            >
              You placed into Level {result.placement.assignedLevel}:{" "}
              {result.placement.assignedLevelName}. This unlocks{" "}
              {result.user.unlockedDifficulties.join(" + ")} topics to start
              training.
            </div>
          </div>
      ...
      <div style={{ display: "grid", gap: "12px", marginBottom: "18px" }}>
        {scores.map((score, index) => (
          <div
            key={score.miniGame}
            style={{ ...sectionCard, padding: "18px 20px" }}
          >
            <div
              style={{
                display: "flex",
...
export default function TutorialPlacementScreen({ onComplete }) {
  ...
  const submit = async () => {
    if (!session || submitting || skipSubmitting) return;
  ...
  const skipTutorial = async () => {
    ...
    const confirmed = window.confirm(
      "Skip tutorial and start at the lowest rank?",
    );
    if (!confirmed) return;
    ...
    try {
      const response = await apiFetch("/tutorial/skip", {
        method: "POST",
        body: JSON.stringify({
          sessionId: session.id,
        }),
  ...
  if (loading) {
    return (
      <div style={{ ...pageWrap, color: "#667085", fontSize: "14px" }}>
        Loading tutorial…
      </div>
    );
  }
    ...
    return (
      <div style={pageWrap}>
        <div style={{ color: "#dc2626", fontSize: "13px" }}>
          {error || "Tutorial could not be loaded."}
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={pageWrap}>
      <div style={{ ...heroCard, marginBottom: "18px" }}>
        ...
        >
          You’ll complete three short placement exercises. The app samples one
          prompt from each backend training bank to estimate your starting level.
        </p>
        <div
          style={{
            display: "flex",
      ...
      <div style={{ ...sectionCard, padding: "22px", marginBottom: "16px" }}>
        ...
        >
          <div>
            <div style={eyebrowSmall}>
              Exercise {current + 1} of {session.questions.length}
            </div>
            <div
              style={{
                fontSize: "24px",
        ...
        {question.category && (
          ...
          >
            <div style={{ fontSize: "13px", color: "#475467", lineHeight: 1.65 }}>
              {question.category}
            </div>
          </div>
        )}
          ...
          <div style={{ display: "grid", gap: "10px", marginBottom: "14px" }}>
            {question.options.map((option, index) => (
              <button
                key={option}
                onClick={() =>
                  updateAnswer({ selectedOption: option, selectedIndex: index })
                }
      ...
      <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
        ...
        {!isLast && (
          <button
            onClick={() => setCurrent((prev) => prev + 1)}
            disabled={!canContinue || submitting || skipSubmitting}
            style={{
              ...solidBtn,
              opacity: canContinue && !submitting && !skipSubmitting ? 1 : 0.5,
            }}
          >
            Next
          </button>
      ...
      <button
        ...
        style={{
          ...
          padding: "12px 16px",
          borderRadius: "999px",
          boxShadow: "0 16px 30px rgba(17, 24, 39, 0.18)",
          opacity: submitting || skipSubmitting ? 0.5 : 1,
        }}
      >
        {skipSubmitting ? "Skipping…" : "Skip tutorial"}
      </button>
    </div>
  );
}